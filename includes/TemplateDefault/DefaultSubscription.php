...
namespace YayMailAddon\TemplateDefault;
...
class DefaultSubscription {
	...
	public static function get_templates( $custom_order, $email_heading ) {
		...
		*/
		...
		$custom_text           = esc_html__( ' has been suspended by the user.', 'woocommerce' );
		$email_text            = esc_html__( 'A subscription belonging to ', 'woocommerce' ) . esc_html( do_shortcode( '[yaymail_billing_first_name] [yaymail_billing_last_name]' ) ) . $custom_text . esc_html__( ' Their subscription\'s details are as follows:', 'woocommerce' );
		$additional_content    = __( 'Thanks for reading.', 'woocommerce' );
		$text_shipping_address = __( 'Shipping Address', 'woocommerce' );
		$text_billing_address  = __( 'Billing Address', 'woocommerce' );
		...
		*/
		...
		},
		{
			...
			"settingRow": {
				...
				"titleBilling": "' . $text_billing_address . '",
				"titleShipping": "' . $text_shipping_address . '",
		...
		},
		{
			...
			"nameElement": "Text",
			"settingRow": {
				"content": "<p><span style=\"font-size: 14px;\">' . $additional_content . '</span></p>",
				"backgroundColor": "#fff",
				"textColor": "#636363",
				"family": "Helvetica,Roboto,Arial,sans-serif",
				"paddingTop": "0",
